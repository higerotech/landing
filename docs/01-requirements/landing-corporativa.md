...
| D1b | El apex `higerotech.com` **no resolvía** | ✅ **Cerrada el 2026-07-31.** El apex y `www` se sirven desde un Worker de Cloudflare (ADR-0006) y salieron del ingress del túnel: responde 200 donde daba 530. El `canonical`, los tres `hreflang`, `og:url`, el `sitemap.xml` y `robots.txt` ya señalan a un host que existe, y el contenido duplicado queda consolidado. `web.` y `demo.` siguen en el túnel como contingencia |
...
| D4 | Terminación TLS en el borde | ✅ **Cerrada.** La termina Cloudflare; HSTS activo desde el 2026-07-31 con los cuatro requisitos de precarga cumplidos. El tramo en HTTP plano (T9) **solo queda en el camino de contingencia**: en el del Worker no existe |
| R1 | Sin observabilidad: una caída pasa inadvertida | **Abierto** — Gate 5. Servir desde el borde quita de en medio el host doméstico, pero eso reduce la exposición, **no la ceguera**: sigue sin haber alerta que avise |
...
| R4 | **Una excepción temprana en el script inline deja la página en blanco** | **Abierto**, detectado el 2026-07-30 al diseñar las unitarias. Registrado como amenaza **T17** en `threat-model.md`, con DREAD 5,8. `.reveal` está en `opacity: 0` esperando que el JS le añada `.in`, y las líneas 919, 920, 972, 973 y 989 desreferencian nodos sin guarda: una errata en un `id` blanquea el sitio. Las tres capas de ADR-0005 **no lo alcanzan** —la del `<noscript>` solo actúa si el JS está deshabilitado, y la rama de respaldo vive dentro del script, en la línea 978, así que una excepción anterior la deja inalcanzable—. Detección diseñada: U1/U2 de `docs/04-testing/unit-tests.md`. Prevención real: la 4.ª alternativa de ADR-0005, descartada solo por el parpadeo |
